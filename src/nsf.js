...
const soundLoad = [
	...
	0x8D, 0x01, 0x02, 0x0A, 0xA8, 0xB9, 0x06, 0x82, 0x85, 0x00, 0xB9, 0x07,
...
const playFrame = [
	...
	0x02, 0x9D, 0x45, 0x02, 0x20, 0xB3, 0x80, 0x20, 0x6C, 0x81, 0xE8, 0xE0,
	0x06, 0xD0, 0xE3, 0x20, 0xA1, 0x81, 0x60
];

const fetchByte = [
	// Offset 0x00000133 to 0x000001D5
	...
	0x80, 0x9D, 0x33, 0x02, 0x4C, 0x47, 0x81, 0x29, 0x7F, 0x8C, 0x01, 0x02,
	...
	0x8D, 0x02, 0x02, 0xBD, 0x21, 0x02, 0xC9, 0x03, 0xD0, 0x06, 0x20, 0x56,
	0x81, 0x4C, 0x47, 0x81, 0xAD, 0x02, 0x02, 0x9D, 0x3F, 0x02, 0xC8, 0xB1,
	0x00, 0x9D, 0x39, 0x02, 0xC8, 0x98, 0x18, 0x7D, 0x27, 0x02, 0x9D, 0x27,
	0x02, 0x90, 0x03, 0xFE, 0x2D, 0x02, 0x60
];

const doNoise = [
	// Offset 0x000001D6 to 0x000001EB
	0xAD, 0x02, 0x02, 0x29, 0x10, 0xF0, 0x08, 0xAD, 0x02, 0x02, 0x09, 0x80,
	0x8D, 0x02, 0x02, 0xAD, 0x02, 0x02, 0x9D, 0x39, 0x02, 0x60
];

const setTempPorts = [
	// Offset 0x000001EC to 0x00000220
...
const setApu = [
	// Offset 0x00000221 to 0x00000285
...
export let nsfDriver = header.concat(load, init, silence, soundLoad,
  playFrame, fetchByte, doNoise, setTempPorts, setApu,
...
export function assembleDriver(sq1, sq2, t, n) {
  ...
  tri = t;
  noise = n;
  const s2lo = fmtWord((sq1.length + 0x8221))[0];
  const s2hi = fmtWord((sq1.length + 0x8221))[1];
  const s3lo = fmtWord((sq1.length + sq2.length + 0x8221))[0];
  const s3hi = fmtWord((sq1.length + sq2.length + 0x8221))[1];
  const s4lo = fmtWord((sq1.length + sq2.length + t.length + 0x8221))[0];
  const s4hi = fmtWord((sq1.length + sq2.length + t.length + 0x8221))[1];
  streams = [
    MUSIC_SQ1, 1, SQUARE_1, 0xBC, 0x21, 0x82,
