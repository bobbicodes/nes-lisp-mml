...
import { s1, s2, s3, s4, s5, s6, s7, s8, addVolEnvelope, resetVolEnvelopes, currentVolEnvelopes, addPitchEnvelope, resetPitchEnvelopes, currentPitchEnvelopes, addDutyEnvelope, resetDutyEnvelopes, currentDutyEnvelopes, addArp, resetArps, currentArps } from "./nsf"

const END_STREAM = 0xA0
const CHANGE_VOL = 0xA2
const CHANGE_DUTY = 0xA3
const CHANGE_VIB = 0xAA
const CHANGE_ARP = 0xAB
const CHANGE_DUTY_ENV = 0xAC
const LOOP1 = 0xA5
const LOOP1SET = 0xA4
const LOOP2 = 0xA7
const LOOP2SET = 0xA6
const SET_DURATION = 0xAD
...
let loop1Total = 0
let loop2Total = 0
let streamPtrs = [s1, s2, s3, s4, s5, s6, s7, s8]
let loop1Point = 0
let loop2Point = 0
...
function compileLoop2Set(loop, streamNum) {     
  ...
  console.log("loop2Point:", hex(loop2Point))
...
function compileLength(length) {
  ...
  if (length > 25) {
    stream.push(SET_DURATION, length)
  } else {
    stream.push(0x80 + length)
  }
  currentLength = length
}

function compileVolumeEnvelope(envelope) {
  // check if envelope is already added, add it if not
  if (!containsEnvelope(envelope, currentVolEnvelopes)) {
    addVolEnvelope(envelope)
  }
  // containsEnvelope returns its index, which we use to push an env change
  let envNum = containsEnvelope(envelope, currentVolEnvelopes)
  stream.push(CHANGE_VOL, envNum)
}
...
function compileDutyEnv(duty, streamNum) {
  // check if envelope is already added, add it if not
  if (!containsEnvelope(duty, currentDutyEnvelopes)) {
    addDutyEnvelope(duty)
  }
  // containsEnvelope returns its index, which we use to push an env change
  let envNum = containsEnvelope(duty, currentDutyEnvelopes)
  stream.push(CHANGE_DUTY_ENV, envNum)
}
...
function fmtWord2(n) {
  ...
  n = Math.round(n)
  const pad = String(n.toString(16)).padStart(4, '0');
  return [parseInt(pad.slice(0, 2), 16),
          Math.max(1, parseInt(pad.slice(2), 16))];
}

function compileArp(arp, streamNum) {
  let pitches = []
  for (let i = 0; i < arp.length; i++) {
    if (streamNum === 7) {
      pitches = pitches.concat(fmtWord2(sawFreqToPeriod(midiToFreq(arp[i]))))
    } else {
      pitches = pitches.concat(fmtWord2(freqToPeriod(midiToFreq(arp[i]))))
    }
  }
  // check if arp is already added, add it if not
  if (!containsEnvelope(pitches, currentArps)) {
    addArp(pitches)
  }
  // containsEnvelope returns its index, which we use to push an arp change
  let arpNum = containsEnvelope(pitches, currentArps)
  stream.push(CHANGE_ARP, arpNum)
}
...
export function compileStream(noteSequence, streamNum) {
    ...
    for (let i = 0; i < notes.length; i++) {
      ...
      if (notes[i].has("ʞenvelope")) {
        compileVolumeEnvelope(notes[i].get("ʞenvelope"))
      }
      if (notes[i].has("ʞvolume")) {
        compileVolume(notes[i].get("ʞvolume"))
      }
      if (notes[i].has("ʞduty")) {
        if (Array.isArray(notes[i].get("ʞduty"))) {
          compileDutyEnv(notes[i].get("ʞduty"), streamNum)
        } else {
